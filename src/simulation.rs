...
use crate::{
    atom::{AtomPlugin, ClearForceSystem},
    destructor::DestroyAtomsPlugin,
    gravity::GravityPlugin,
    integrator::{
        Step, VelocityVerletIntegratePositionSystem, VelocityVerletIntegrateVelocitySystem,
...
impl SimulationBuilder {
    pub fn new() -> Self {
        ...
        dispatcher_builder.add(ClearForceSystem, "clear", &[INTEGRATE_POSITION_SYSTEM_NAME]);
    ...
    fn check_plugin_dependencies(&self, plugin: &impl Plugin) {
        for dep in plugin.deps() {
            ...
            {
                panic!(
                    "Cannot add plugin {}: it requires a {} plugin, which has not yet been added.",
                    plugin.name(),
                    dep.name()
                );
            }
